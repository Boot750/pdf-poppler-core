...
# Build poppler binaries compatible with AWS Lambda (Amazon Linux 2, GLIBC 2.26)
# This image creates binaries that work on Lambda without GLIBC version errors

# Install dependencies (Amazon Linux 2 uses yum)
...
# Detect and record poppler version
RUN POPPLER_VERSION=$(LD_LIBRARY_PATH=/output/lib /output/bin/pdfinfo -v 2>&1 | grep -oP '\d+\.\d+' | head -1 || echo "unknown") && \
    echo "POPPLER_VERSION=${POPPLER_VERSION}" > /output/VERSION && \
    echo "VARIANT=xvfb" >> /output/VERSION && \
    echo "PLATFORM=aws-2" >> /output/VERSION && \
    echo "GLIBC=2.26" >> /output/VERSION && \
    echo "BUILD_DATE=$(date -I)" >> /output/VERSION && \
    echo "Detected poppler version: ${POPPLER_VERSION} (AWS Lambda compatible)"

WORKDIR /output

# Create tar.gz with both poppler and Xvfb, including VERSION file
CMD ["sh", "-c", "tar -czf /tmp/poppler-aws2-binaries.tar.gz bin lib share VERSION && cat /tmp/poppler-aws2-binaries.tar.gz"]
