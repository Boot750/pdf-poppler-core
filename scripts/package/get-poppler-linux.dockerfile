...
RUN dnf install -y poppler-utils mesa-libEGL mesa-libGL mesa-libGLU
...
# Output tar includes VERSION file for version detection
CMD tar -czf /tmp/poppler-linux-lambda-binaries.tar.gz bin lib VERSION 2>/dev/null || tar -czf /tmp/poppler-linux-lambda-binaries.tar.gz bin VERSION