...
RUN POPPLER_VERSION=$(LD_LIBRARY_PATH=/output/lib /output/bin/pdfinfo -v 2>&1 | grep -oP '\d+\.\d+' | head -1 || echo "unknown") && \
    ...
    echo "Detected poppler version: ${POPPLER_VERSION} (xvfb variant)"

WORKDIR /output

# Create tar.gz with both poppler and Xvfb, including VERSION file
CMD ["sh", "-c", "tar -czf /tmp/poppler-xvfb-linux-binaries.tar.gz bin lib share VERSION && cat /tmp/poppler-xvfb-linux-binaries.tar.gz"]